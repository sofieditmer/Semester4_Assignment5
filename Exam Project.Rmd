...
summary(m1_SCS)
inv_logit(-0.12) 
# This means that when going from being a collectivist to an individualist, then it becomes less likely to defect than to cooperate - -0.12 log odds which is 47%. 
...
summary(m2_SCS)
inv_logit(-0.0)
# 50.25%- thus, the continious SCS-score is a worse predictor of choice than the binary SCS was
...
Using all the data (all dilemmas)
```{r}
# Creating Bayesian formulas for each model: 
...
# Binary SCS as predictor
SCS_f1 <- bf(choice ~ 1 + SCS)

# Continuous SCS as predictor
SCS_f2 <- bf(choice ~ 1 + SCS_score)
...
# Model 1: SCS model (binary) #
get_prior(SCS_f1, d = d_long, family = bernoulli())
...
# Trace plots and trace rank plots
color_scheme_set("viridis")
mcmc_trace(m1,  pars = "b_Intercept", "b_SCSindividualist") + theme_classic()
mcmc_rank_overlay(m1, pars = "b_Intercept", "b_SCSindividualist") + theme_classic()


# Hypothesis-testing
hypothesis(m1, "SCSindividualist > 0")
        
# Model 2: Continuous SCS score predictor # 
get_prior(SCS_f2, d = d_long, family = bernoulli())
...
# Trace plots and trace rank plots
color_scheme_set("viridis")
mcmc_trace(m2,  pars = "b_Intercept", "b_SCS_score") + theme_classic()
mcmc_rank_overlay(m2, pars = "b_Intercept", "b_SCS_score") + theme_classic()

# Hypothesis-testing
hypothesis(m2, "SCS_score > 0")
...
conditional_effects(m1) # people genreally defect more in the classic condition compared to the two others
